...
public:
	...
	* @return ERR_SUCC 成功 其他失败
	*/
	errno_t set_torsensor_brand(int sensor_brand);
	...
	* @return ERR_SUCC 成功 其他失败
	*/
	errno_t get_torsensor_brand(int *sensor_brand);
	...
	* @param period 当电流指令中断时，沿用上一有效指令的周期数；0 不沿用，电流自动切换为机器人当前重力矩
	* @return ERR_SUCC 成功 其他失败
	*/
	errno_t enable_torque_control(int enable, int period);
	...
	* @return ERR_SUCC 成功 其他失败
	*/
	errno_t set_torque_control_value(const JointTorque* torque);
	...
	*/
	...
	/**
	* @brief 设置7轴机器人逆解形式
	* @param flag 0, 固定臂型角，臂型角为当前角，1 指定臂型角，臂型角为指定角，2 臂型角随动避障
	* @param arm_angle 指定臂型角
	* @return ERR_SUCC 成功 其他失败
	*/
	errno_t set_7_dof_inv_inform(int flag, double arm_angle);
	...
	* @return ERR_SUCC 成功 其他失败
	*/
	errno_t set_7_dof_avoid_limit_param(double flag, double arm_angle);

	/**
	* @brief 执行运动轨迹文件
	* @return ERR_SUCC 成功 其他失败
	*/
	errno_t start_servo_traj(const char* filename, BOOL is_block, BOOL is_joint);

	/**
	* @brief 设置力控死区
	* @param dead_zone Fx, Fy, Fz, rx, ry, rz
	* @return ERR_SUCC 成功 其他失败
	*/
	errno_t set_force_control_dead_zone(const double* dead_zone);

	/**
	* @brief 设置控制器ip
	* @param DHCPorStatic 1 for static; 0 for DHCP
	* @param ip 
	* @param mask
	* @param gateway
	* @return ERR_SUCC 成功 其他失败
	*/
	errno_t set_controller_LAN2(int DHCPorStatic, const char* ip, const char* mask, const char* gateway);

	/**
	* @brief 获取信号量值
	* @param sign_info_array
	* @param index  信号量序号
	* @return ERR_SUCC 成功 其他失败
	*/
	errno_t get_signal_by_index(SignInfo* sign_info_array, int index);
