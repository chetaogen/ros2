...
using arm_hand_interface::action::DexterousHand;
...
public:
    robot_control() : Node("robot_control")
    {
        ...
        this->m_dex_server_ = rclcpp_action::create_server<DexterousHand>(
            this, "DexterousHand",
            std::bind(&robot_control::handle_goal_hand, this, _1, _2),
            std::bind(&robot_control::handle_cancel_hand, this, _1),
            std::bind(&robot_control::handle_accepted_hand, this, _1));
...
public:
    ...
    rclcpp_action::GoalResponse handle_goal_hand(const rclcpp_action::GoalUUID &uuid, std::shared_ptr<const DexterousHand::Goal> goal);
    rclcpp_action::CancelResponse handle_cancel_hand(const std::shared_ptr<GoalHandleDexterousHand> goal_handle);
    void handle_accepted_hand(const std::shared_ptr<GoalHandleDexterousHand> goal_handle);
    void execute_move_hand(const std::shared_ptr<GoalHandleDexterousHand> goal_handle);
    ...
    std::vector<Action> actions = {action0, action1, action2, action3, action4, action5};
    std::vector<Judge> judges = {judge0, judge1, judge2, judge3, judge4, judge5};
...
void robot_control::publish_joint_states(std::vector<double> SampledJointPosition)
{
    if (rclcpp::ok())
    {
        ...
        jointStates.name = {"r-j1_right", "r-j2_right", "r-j3_right", "r-j4_right", "r-j5_right", "r-j6_right"};
...
void robot_control::handle_accepted(const std::shared_ptr<GoalHandleFJT> goal_handle)
{
    ...
    std::thread{std::bind(&robot_control::execute_move, this, _1), goal_handle}.detach();
}
...
void robot_control ::publisher_timer_3_Callback()
{
    ...
    /*** 右臂和右手的状态读取，并发布topic ***/
    {
        ...
        arm_status.arm_no = 2; // 右臂
        //arm_status.status_no = count1;
        arm_status.time = rclcpp::Clock().now();
        ...
        for (int k = 0; k <= 5; k++)
            arm_status.arm_joint_angles[k] = SampledJointPosition[k];



        tf2::Quaternion q;
        CartesianPose tcp_position;
        Get_Tcp_Position(&tcp_position);
        q.setRPY(tcp_position.rpy.rx, tcp_position.rpy.ry, tcp_position.rpy.rz);

        arm_status.arm_pose.position.x = tcp_position.tran.x / 1000.0; // 注意 mm 要转化为 m
        arm_status.arm_pose.position.y = tcp_position.tran.y / 1000.0;
        arm_status.arm_pose.position.z = tcp_position.tran.z / 1000.0;
        arm_status.arm_pose.orientation.x = q.x();// 单位都是 rad
        arm_status.arm_pose.orientation.y = q.y();               // 单位都是 rad
        arm_status.arm_pose.orientation.z = q.z();               // 单位都是 rad
        arm_status.arm_pose.orientation.w = q.w();
        ...
        arm_status.arm_motion_status = in_pos;
        ...
        for (int k = 0; k <= 5; k++)
        {
            arm_status.hand_angles[k] = R_ByteArray[k]; // 需要转换为角度
          //  RCLCPP_INFO(this->get_logger(), " R_ByteArray[k]= %d",R_ByteArray[k]);
        }

        
        arm_status.arm_collision_status = Is_In_Collision();


        arm_status.hand_forces ={0,0,0,0,0,0}; //手指受力，待补充

        m_arm_status_publisher->publish(arm_status);
        
    }
}

rclcpp_action::GoalResponse robot_control ::handle_goal_hand(const rclcpp_action::GoalUUID &uuid, std::shared_ptr<const DexterousHand::Goal> goal)
{
    (void)uuid;
...
rclcpp_action::CancelResponse robot_control ::handle_cancel_hand(const std::shared_ptr<GoalHandleDexterousHand> goal_handle)
{
    return rclcpp_action::CancelResponse::ACCEPT;
}

void robot_control ::handle_accepted_hand(const std::shared_ptr<GoalHandleDexterousHand> goal_handle)
{
    using std::placeholders::_1;

    std::thread{std::bind(&robot_control::execute_move_hand, this, _1), goal_handle}.detach();
}

void robot_control ::execute_move_hand(const std::shared_ptr<GoalHandleDexterousHand> goal_handle)
{
    static long count = 0;
    count++;
    ...
    if (goal_handle->get_goal()->hand_no == 1) // 左手  根据左右手的编号不同，使用不同的write函数来控制不同的手指
    {
        // goal.hand[1];             goal.hand[0]?                     // 手的动作序列,赋值给write_SeriesPort_modbus函数
       int j = goal_handle->get_goal()->hand[1] ;
       actions[j]();

        auto feedback = std::make_shared<DexterousHand::Feedback>();
       // std::vector<uint16_t> R_ByteArray;
        rclcpp::Rate rate1(1); // 10hz
        ...
        do
        {
            i++;
            if(i>=10)
                break;// 如果10s 还执行不完，就认为手执行失败
            // 给action的客户端发送实时状态反馈
            feedback->hand_no = goal_handle->get_goal()->hand_no;
            //  读取手的关节角度
          // std::vector<uint16_t> R_ByteArray = read_SeriesPort();// 读取手的6关节角度 
           R_ByteArray = read_SeriesPort();
           //bool b =judges[k](R_ByteArray[0],R_ByteArray[1],R_ByteArray[2],R_ByteArray[3],R_ByteArray[4],R_ByteArray[5]);

            for (int k = 0; k <= 5; k++)
                feedback->hand_pose[k] = R_ByteArray[k]; // 需要转换为角度

            goal_handle->publish_feedback(feedback); // 过程反馈
            rate1.sleep();
        }while  ( judges[j](R_ByteArray[0],R_ByteArray[1],R_ByteArray[2],R_ByteArray[3],R_ByteArray[4],R_ByteArray[5])); 
        ...
        result->hand_no = goal_handle->get_goal()->hand_no;

        R_ByteArray = read_SeriesPort(); // 读取手的6关节角度
        for (int k = 0; k <= 5; k++)
            result->hand_pose[k] = R_ByteArray[k]; // 需要转换为角度
    ...
    else if (goal_handle->get_goal()->hand_no == 2) // 右手
    {
       // goal.hand[1];             goal.hand[0]?                     // 手的动作序列,赋值给write_SeriesPort_modbus函数
       int j = goal_handle->get_goal()->hand[1] ;
       actions[j]();
