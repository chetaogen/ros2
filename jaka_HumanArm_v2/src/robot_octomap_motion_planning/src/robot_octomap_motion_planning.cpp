...
using  arm_hand_interface::action::ArmVisionPlanning;
using  arm_hand_interface::msg::ArmStatus;
...
public:
    octomap_planning() : Node("octomap_planning")
    {
       ...
       // m_publisher = this->create_publisher<octomap_msgs::msg::Octomap>("pc", 10);
        ...
        /*************** 创建服务端，接收 ArmVisionPlanning.action ************/
        this->m_arm_vision_planning_server = rclcpp_action::create_server<ArmVisionPlanning>(
            this, "ArmVisionPlanning",
            std::bind(&octomap_planning::handle_goal, this, _1, _2),
            std::bind(&octomap_planning::handle_cancel, this, _1),
            std::bind(&octomap_planning::handle_accepted, this, _1));
    }
    ...
    void octomap_topic_subscribe( const octomap_msgs::msg::Octomap &msg);                                                // 订阅障碍物的topic
    void arm_status_topic_subscribe(const ArmStatus& arm_status) ;
...
public:
    ...
    geometry_msgs::msg::Pose m_arm_left_target_pose; // 左机械臂期望位姿
    geometry_msgs::msg::Pose m_arm_right_target_pose; // 右机械臂期望位姿
...
void octomap_planning::handle_accepted(const std::shared_ptr<GoalHandleArmVisionPlanning> goal_handle)
{
    ...
    RCLCPP_INFO(this->get_logger(), "这是 handle_accepted");
    ...
    if ( goal_handle->get_goal()->arm_no == 1) //左臂
    {
        ...
        m_arm_left_target_pose = goal->arm_target_pose;
    ...
    else if (goal_handle->get_goal()->arm_no == 2) // 右臂
    {
        ...
        m_arm_right_target_pose = goal->arm_target_pose;
...
void octomap_planning :: publisher_timerCallback1()
{
    ...
    if(!m_arm_left_motion_success) // 左臂是否完成运动的变量 m_arm_left_motion_success, 赋值在motion_planning_interface函数中实现
    {
        // 如果收到 取消action的请求
        ...
        for (int i = 0; i <= 5; i++)
            feedback->arm_joint_angles[i] = m_arm_left_jointAngles[i]; // 左臂实际关节角度
    ...
    else if (rclcpp::ok()) //结果反馈
    {
        ...
        RCLCPP_INFO(this->get_logger(), "左臂运动任务完成!");
...
void octomap_planning ::publisher_timerCallback2()
{
    ...
    RCLCPP_INFO(this->get_logger(), "timerCallback2");

    const auto goal = m_myactionPtr2->get_goal();
    auto feedback = std::make_shared<ArmVisionPlanning::Feedback>();
    auto result = std::make_shared<ArmVisionPlanning::Result>();

    if (!m_arm_right_motion_success) // 右臂是否完成运动的变量 m_arm_right_motion_success, 赋值在 motion_planning_interface函数中实现
    {
        // 如果收到 取消action的请求
        ...
        //feedback->command_no = goal->command_no;
         ...
         RCLCPP_INFO(this->get_logger(), "m_arm_right_motion_success");

        feedback->plan_success = m_arm_right_plan_success; // 赋值在 motion_planning_interface函数中实现
        //feedback->time_from_start = rclcpp::Clock().now() - m_begin_time;
        feedback->arm_pose = m_arm_right_pose;                // 右臂实际位姿

        for (int i = 0; i <= 5; i++)
            feedback->arm_joint_angles[i] = m_arm_right_jointAngles[i]; // 右臂实际关节角度


        m_myactionPtr2->publish_feedback(feedback); // 提供实时反馈

        RCLCPP_INFO(this->get_logger(), "右臂运动连续反馈中");
    }
    ...
    else if (rclcpp::ok())
    {
       ...
       // result->time_from_start = rclcpp::Clock().now() - m_begin_time;
        ...
        RCLCPP_INFO(this->get_logger(), "右臂运动任务完成!");
...
void octomap_planning ::motion_planning_interface(int arm_no, octomap_msgs::msg::Octomap octomap_msg, geometry_msgs::msg::Pose target_pose)
{
    ...
    if (arm_no == 1) //左臂的避障规划
    {
        ...
        if (success)
        {
            //m_arm_left_motion_success = static_cast<bool>(arm_move_group.execute(plan));
            moveit::core::MoveItErrorCode  result = arm_move_group.execute(plan);
            
            if (result == moveit::core::MoveItErrorCode::SUCCESS) 
            {
//m_arm_left_motion_success = true;
RCLCPP_INFO(this->get_logger(), "Execute success");
    ...
    if( arm_no == 2 ) //右臂的避障规划
    {
        ...
        if (success)
        {
            ...
            if (result == moveit::core::MoveItErrorCode::SUCCESS)
            {
             //   m_arm_right_motion_success = true;
                RCLCPP_INFO(this->get_logger(), "Execute success");
            }
...
void octomap_planning ::octomap_topic_subscribe(const octomap_msgs::msg::Octomap &msg)
{
    octomap_msgs::msg::Octomap octomap_msg = msg;
    m_octomap_msg = msg;
};

void octomap_planning ::arm_status_topic_subscribe(const ArmStatus &arm_status)
{
    ...
    {
        m_arm_left_pose = arm_status.arm_pose; // 左机械臂实际位姿

        for (int i = 0; i <= 5; i++)
            m_arm_left_jointAngles[i] = arm_status.arm_joint_angles[i]; // 左机械臂实际关节角度
    }
    ...
    else if (arm_status.arm_no == 2)//右臂
    {
        ...
        // RCLCPP_ERROR(this->get_logger(), "---------------------arm_status_topic_subscrib--------------");
        m_arm_right_pose = arm_status.arm_pose; // 右机械臂实际位姿
RCLCPP_ERROR(this->get_logger(), "m_arm_right_target_pose.position.x  %.3f", m_arm_right_target_pose.position.x );
RCLCPP_ERROR(this->get_logger(), "m_arm_right_pose.position.x = %.3f",  m_arm_right_pose.position.x);
//RCLCPP_ERROR(this->get_logger(), "abs(m_arm_right_target_pose.position.x - m_arm_right_pose.position.x) = %.3f", abs(m_arm_right_target_pose.position.x - m_arm_right_pose.position.x));
RCLCPP_ERROR(this->get_logger(), "m_arm_right_target_pose.position.y  %.3f", m_arm_right_target_pose.position.y );
RCLCPP_ERROR(this->get_logger(), "m_arm_right_pose.position.y = %.3f",  m_arm_right_pose.position.y);
//RCLCPP_ERROR(this->get_logger(), "abs(m_arm_right_target_pose.position.y - m_arm_right_pose.position.y) = %.3f", abs(m_arm_right_target_pose.position.y - m_arm_right_pose.position.y));
RCLCPP_ERROR(this->get_logger(), "m_arm_right_target_pose.position.z  %.3f", m_arm_right_target_pose.position.z );
RCLCPP_ERROR(this->get_logger(), "m_arm_right_pose.position.z = %.3f",  m_arm_right_pose.position.z);
//RCLCPP_ERROR(this->get_logger(), "abs(m_arm_right_target_pose.position.z - m_arm_right_pose.position.z) = %.3f", abs(m_arm_right_target_pose.position.z - m_arm_right_pose.position.z));

RCLCPP_ERROR(this->get_logger(), "m_arm_right_target_pose.orientation.x  %.3f", m_arm_right_target_pose.orientation.x );
RCLCPP_ERROR(this->get_logger(), "m_arm_right_pose.orientation.x = %.3f",  m_arm_right_pose.orientation.x);
//RCLCPP_ERROR(this->get_logger(), "abs(m_arm_right_target_pose.orientation.x - m_arm_right_pose.orientation.x) = %.3f", abs(m_arm_right_target_pose.orientation.x - m_arm_right_pose.orientation.x));
RCLCPP_ERROR(this->get_logger(), "m_arm_right_target_pose.orientation.y  %.3f", m_arm_right_target_pose.orientation.y );
RCLCPP_ERROR(this->get_logger(), "m_arm_right_pose.orientation.y = %.3f",  m_arm_right_pose.orientation.y);
//RCLCPP_ERROR(this->get_logger(), "abs(m_arm_right_target_pose.orientation.y - m_arm_right_pose.orientation.y) = %.3f", abs(m_arm_right_target_pose.orientation.y - m_arm_right_pose.orientation.y));
RCLCPP_ERROR(this->get_logger(), "m_arm_right_target_pose.orientation.z  %.3f", m_arm_right_target_pose.orientation.z);
RCLCPP_ERROR(this->get_logger(), "m_arm_right_pose.orientation.z = %.3f",  m_arm_right_pose.orientation.z);
//RCLCPP_ERROR(this->get_logger(), "abs(m_arm_right_target_pose.orientation.z - m_arm_right_pose.orientation.z) = %.3f", abs(m_arm_right_target_pose.orientation.z - m_arm_right_pose.orientation.z));
RCLCPP_ERROR(this->get_logger(), "m_arm_right_target_pose.orientation.w  %.3f", m_arm_right_target_pose.orientation.w);
RCLCPP_ERROR(this->get_logger(), "m_arm_right_pose.orientation.w = %.3f",  m_arm_right_pose.orientation.w);
//RCLCPP_ERROR(this->get_logger(), "abs(m_arm_right_target_pose.orientation.w - m_arm_right_pose.orientation.w) = %.3f", abs(m_arm_right_target_pose.orientation.w - m_arm_right_pose.orientation.w));

RCLCPP_ERROR(this->get_logger(), "arm_status.arm_motion_status: %d", arm_status.arm_motion_status);


          if (abs(m_arm_right_target_pose.position.x - m_arm_right_pose.position.x) <= 0.01 && abs(m_arm_right_target_pose.position.y - m_arm_right_pose.position.y) <= 0.01 && abs(m_arm_right_target_pose.position.z - m_arm_right_pose.position.z) <= 0.01 && abs(m_arm_right_target_pose.orientation.x - m_arm_right_pose.orientation.x) <= 0.01 && abs(m_arm_right_target_pose.orientation.y - m_arm_right_pose.orientation.y) <= 0.01 && abs(m_arm_right_target_pose.orientation.z - m_arm_right_pose.orientation.z) <= 0.01 && arm_status.arm_motion_status == true)
        //if ( count2 >= 300 && arm_status.arm_motion_status == 1)
        {
            m_arm_right_motion_success = true;
        ...
        for (int i = 0; i <= 5; i++)
            m_arm_right_jointAngles[i] = arm_status.arm_joint_angles[i]; // 右机械臂实际关节角度
    }
};
