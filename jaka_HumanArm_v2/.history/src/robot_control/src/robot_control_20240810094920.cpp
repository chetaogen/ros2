...
using ArmVisionPlanningarm_hand_interface::action::ArmVisionPlanning;
using DexterousHandarm_hand_interface::action::DexterousHand;
...
public:
    ...
    void publisher_timer_1_Callback(); // 定时器1的回调函数
    void publisher_timer_2_Callback(); // 定时器2的回调函数
    ...
    // ArmVisionPlanning.action 服务端
    rclcpp_action::Server<ArmVisionPlanning>::SharedPtr m_arm_vision_planning_server_;
...
void robot_control ::publisher_timer_1_Callback()
{
    ...
    // if-else 这段代码用于没有真实机械臂时。如果有真实机械臂，此段代码不用
    /*    if (m_SampledJointStates.size() >= 6)
        {
            for (int i = 0; i <= 5; i++)
                SampledJointPosition.push_back(m_SampledJointStates[i]);

            m_SampledJointStates.erase(m_SampledJointStates.begin(), m_SampledJointStates.begin()+6);
        }
        else
            SampledJointPosition = {0,0,0,0,0,0};
    */
...
void robot_control ::publisher_timer_2_Callback()
{
    ...
    // Joints_Move(&joint_pos, ABS, false, vel_command[maxPosition]);
};
...
void robot_control::execute_move(const std::shared_ptr<GoalHandleFJT> goal_handle)
{
    ...
    for (i = 0; i < point_num-1; i++)
    {
        ...
        /*
                m_SampledJointStates.push_back(goal->trajectory.points[i].positions[0]); // 在没有真实机械臂的情况下，将接收到的关节轨迹作为"joint_states"反馈给robot_state_publisher节点和move_group节点
                m_SampledJointStates.push_back(goal->trajectory.points[i].positions[1]); // 在没有真实机械臂的情况下，将接收到的关节轨迹作为"joint_states"反馈给robot_state_publisher节点和move_group节点
                m_SampledJointStates.push_back(goal->trajectory.points[i].positions[2]); // 在没有真实机械臂的情况下，将接收到的关节轨迹作为"joint_states"反馈给robot_state_publisher节点和move_group节点
                m_SampledJointStates.push_back(goal->trajectory.points[i].positions[3]); // 在没有真实机械臂的情况下，将接收到的关节轨迹作为"joint_states"反馈给robot_state_publisher节点和move_group节点
                m_SampledJointStates.push_back(goal->trajectory.points[i].positions[4]); // 在没有真实机械臂的情况下，将接收到的关节轨迹作为"joint_states"反馈给robot_state_publisher节点和move_group节点
                m_SampledJointStates.push_back(goal->trajectory.points[i].positions[5]); // 在没有真实机械臂的情况下，将接收到的关节轨迹作为"joint_states"反馈给robot_state_publisher节点和move_group节点
        */
