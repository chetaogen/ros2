...
void robot_control ::execute_move_hand(const std::shared_ptr<GoalHandleDexterousHand> goal_handle)
{
    if (goal_handle->get_goal().hand_no == 1) // 左手
    {
        ...
        do
        {
            ...
            bool result = abs(R_ByteArray[0] - 999) <= 10 && abs(R_ByteArray[1] - 999) <= 10 && abs(R_ByteArray[2] - 999) <= 10 && abs(R_ByteArray[3] - 999) <= 10 && abs(R_ByteArray[4] - 1000) <= 10 && abs(R_ByteArray[5] - 1000) <= 10;
