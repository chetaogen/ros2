...
void robot_control ::publisher_timer_3_Callback()
{
    ...
    /*** 右臂和右手的状态读取，并发布topic ***/
    {
        ...
        arm_status.arm_pose.orientation.x = tcp_position.rpy.rx;//单位都是 rad
        arm_status.arm_pose.orientation.y = tcp_position.rpy.ry; // 单位都是 rad
        arm_status.arm_pose.orientation.z = tcp_position.rpy.rz; // 单位都是 rad
        ...
        // arm_status.hand_forces 手指受力，待补充
...
void robot_control ::handle_accepted_hand(const std::shared_ptr<GoalHandleDexterousHand> goal_handle)
{
    ...
    std::thread{std::bind(&robot_control::execute_move_hand, this, _1), goal_handle}
        .detach();
}
...
void robot_control ::execute_move_hand(const std::shared_ptr<GoalHandleDexterousHand> goal_handle)
{
    ...
    if (goal_handle->get_goal()->hand_no == 1) // 左手
    {
        write_SeriesPort_modbus(999, 999, 999, 999, 1000, 1000, ANGLE_SET);//控制手的运动，这里需要修改为对应的手的动作序列

        auto feedback = std::make_shared<DexterousHand::Feedback>();
        std::vector<uint16_t>  R_ByteArray;
        rclcpp::Rate rate1(10); // 10hz
        ...
        do
        {
            ...
            rate1.sleep();
        } while (false == abs(R_ByteArray[0] - 999) <= 10 && abs(R_ByteArray[1] - 999) <= 10 && abs(R_ByteArray[2] - 999) <= 10 && abs(R_ByteArray[3] - 999) <= 10 && abs(R_ByteArray[4] - 1000) <= 10 && abs(R_ByteArray[5] - 1000) <= 10);

        // action的结果反馈
        auto result = std::make_shared<DexterousHand::Result>();
        result->hand_no = goal_handle->get_goal()->hand_no;
        result->command_no = count;
        R_ByteArray = read_SeriesPort(); // 读取手的6关节角度
    ...
    {
       // goal.hand[1];                                    // 手的动作序列,赋值给write_SeriesPort_modbus函数
        write_SeriesPort_modbus(999, 999, 999, 999, 1000, 1000, ANGLE_SET); // 控制手的运动，这里需要修改为对应的手的动作序列
