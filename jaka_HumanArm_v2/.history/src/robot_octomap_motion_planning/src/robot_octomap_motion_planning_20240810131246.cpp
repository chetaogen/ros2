...
public:
    ...
    void motion_planning_interface(int32 arm_no, octomap_msgs::msg::Octomap octomap_msg, geometry_msgs::msg::Pose target_pose); // 利用move_group_interface进行避障轨迹规划
...
public:
    ...
    geometry_msgs::msg::Pose m_arm_pose;//机械臂实际位姿
    float  m_arm_jointAngles[6];//机械臂实际关节角度
    bool   m_arm_plan_success = false;
    bool   m_arm_motion_success =false;
};
...
void octomap_planning::handle_accepted(const std::shared_ptr<GoalHandleArmVisionPlanning> goal_handle)
{
    ...
    motion_planning_interface(m_octomap_msg, target_pose); // 进行避障轨迹规划
...
// 定义一个定时器函数，实现 ArmVisionPlanning 的 action 的连续反馈和结果反馈
void octomap_planning :: publisher_timerCallback()
{
    ...
    if (!m_arm_motion_success) // 机械臂是否完成运动的变量 m_arm_motion_success, 赋值在 ArmStatus.msg 的topic接收函数中实现
    {
        // 如果收到 取消action的请求
        if (m_myactionPtr->is_canceling())
        {
            m_myactionPtr->canceled(result);
            RCLCPP_INFO(this->get_logger(), "任务取消");
            return;
        }
        feedback->arm_no = goal->arm_no;
        feedback->command_no = goal->command_no;
        feedback->plan_success = m_arm_plan_success;
        feedback->time_from_start = rclcpp::Clock().now() - m_begin_time;
        feedback->arm_pose = m_arm_pose; // 机械臂实际位姿
        feedback->arm_joint_angles = m_arm_jointAngles; // 机械臂实际关节角度

        m_myactionPtr->publish_feedback(feedback); // 提供实时反馈

        RCLCPP_INFO(this->get_logger(), "机械臂运动连续反馈中");
    }
    ...
    else if (rclcpp::ok())
    {
        ...
        result->arm_pose = m_arm_pose; // 机械臂实际位姿
        result->success = m_arm_motion_success;
        result->info = NULL;
        m_myactionPtr->succeed(result); // action的结果反馈
...
void octomap_planning ::motion_planning_interface(int32 arm_no, octomap_msgs::msg::Octomap octomap_msg, geometry_msgs::msg::Pose target_pose)
{
    ...
    gripper.setGoalJointTolerance(0.1);
    ...
    m_arm_plan_success = success;

    if (success)
    {
        m_arm_motion_success = static_cast<bool> ( arm_move_group.execute(plan) ); // 作为action的server,发送"/panda_arm_move_group_hand_controller/follow_joint_trajectory" ，消息类型为 control_msgs/action/FollowJointTrajectory
    }
    else
