...
void octomap_planning :: publisher_timerCallback1()
{
    ...
    if(!m_arm_left_motion_success) // 左臂是否完成运动的变量 m_arm_left_motion_success, 赋值在motion_planning_interface函数中实现
    {
        ...
        feedback->arm_pose.position.x = m_arm_left_pose.tran.x / 1000.0; // 注意 mm 要转化为 m
        feedback->arm_pose.position.y = m_arm_left_pose.tran.y / 1000.0;
        feedback->arm_pose.position.z = m_arm_left_pose.tran.z / 1000.0;
        feedback->arm_pose.orientation.x = m_arm_left_pose.rpy.rx; // 单位都是 rad
        feedback->arm_pose.orientation.y = m_arm_left_pose.rpy.ry; // 单位都是 rad
        feedback->arm_pose.orientation.z = m_arm_left_pose.rpy.rz; // 单位都是 rad
...
void octomap_planning ::publisher_timerCallback2()
{
    ...
    if (!m_arm_right_motion_success) // 右臂是否完成运动的变量 m_arm_right_motion_success, 赋值在 motion_planning_interface函数中实现
    {
        ...
        feedback->arm_joint_angles = m_arm_right_jointAngles; // 右臂实际关节角度
